...
boot_redpine <- data_boot_redpine %>% 
  mutate_at(
    vars(harvest, year, inside_slash, inside_fence, plot, species, tag, year, issue),
    factor) %>% 
  mutate(inside_fence = recode(inside_fence, "YES" = "yes")) #old = new


data_sta_rd <- read_csv("tidy_avid_2018-2020_gasline.csv") %>% 
  mutate(issue = " ")  %>% 
  mutate_at(
    vars(harvest, year, inside_slash, inside_fence, plot, species, tag, year, issue),
    factor) %>% 
...
mod_boot_pine <- lm(ht ~ inside_slash * harvest * species * year, 
            data=subset(boot_redpine, species != "bc" & harvest != "sta_rd"))
...
emmip(mod_boot_pine, year ~ inside_slash | species + inside_fence, type = "response", CIs=TRUE)



#emmip = Estimate Marginal Means Interaction Plot
#No Fence vs. Yes Fence, and Without vs. With Slash Wall
emmip(mod_boot_pine, inside_fence ~ inside_slash | species + year, type = "response", CIs=TRUE)
...
emmeans(mod_boot_pine,pairwise ~ inside_fence | inside_slash * species * year, type = "response")


#pairwise "inside_slash" assesses if the slash matters by fence x species x year
emmeans(mod_boot_pine,pairwise ~ inside_slash | inside_fence * species * year, type = "response")
